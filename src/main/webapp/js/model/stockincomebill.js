$(function () {
    ...
    var stockincomebillDialog = $('#stockincomebillDialog'); //弹框
    ...
    window.itsource = {
        add:function(){
            ...
            //3.打开面板(绝对居中)
            stockincomebillDialog.dialog("center").dialog('open').dialog("setTitle","添加数据");
        ...
        edit:function () {
            ...
            if(row){
                ...
                //3.打开面板(绝对居中)
                stockincomebillDialog.dialog("center").dialog('open').dialog("setTitle","修改数据");
                //4.数据进行回显
                //if(row.department){
        ...
        save:function () {
            ...
            //拿到表单中的id
            var id = $("#stockincomebillId").val();
            if(id){
                url = "/stockincomebill/update?cmd=update"
            }
            //提交表单的功能
                ...
                success: function(result){
                   ...
                   if(result.success){
                       //成功之后的处理
                        ...
                        //关闭弹出框
                        stockincomebillDialog.dialog('close');
                   }else{
          ...
          //1.判断是否已经选中了一条数据
            ...
            if(row){
                ...
                $.messager.confirm('确认', '你确定要<span style="color:red;font-size: 20px;">狠心</span>删除我嘛？', function(r){
                    if (r){
                        //3.直接进行相应的删除
                        $.get("/stockincomebill/delete",{id:row.id},function (result) {
                            if(result.success){
                                //删除成功，刷新页面
                                datagrid.datagrid("reload");
                            }else {
